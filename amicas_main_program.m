                            % MAIN PROGRAM FOR BENCHMARK PATIENT SIMULATOR
                                    % Ghent University, JUNE 2026
                                       % ERC AMICAS 2022-2027

clearvars; clear memory; close all; clc;

% variable initialization
BIS_all= []; RASS_all = []; NOL_all = []; CO_all = []; MAP_all = []; NMB_all = [];  

Ts = 1/60; % sampling time 1 seconds (1/60 min)
ST = 0.1*Ts; % simulation step size 

% initialize patient independent models
amicas_hemodyn_model      % hemodynamic models
amicas_dist_model         % disturbance models

%% Choose patient database -> database_type = 1 (for 12 patient database - young patients), database_type = 2 (for 24 patient database - old patients) database_type = 3 (VitalDB mean patients)
database_type = 1;
...
%% Choose input values
Propofol = 0;       % [mg/kg/min] 
Remifentanil = 0.5;     % [ug/kg/min] 
...
for index = 1 : noOfPatients
    ...
    amicas_anesthesia_model   % anesthetic models
    amicas_estimation_adaptation
    % run simulation
    sim('amicas_simulator_v1.slx');
...
subplot(3,2,3) 
plotValues(NOL_all, OutputSignalType.NOL, false, false);
subplot(3,2,4) 
...
warning('off', 'Simulink:Blocks:GotoTagNotFound')